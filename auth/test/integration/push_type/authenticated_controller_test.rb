...
class AuthenticatedControllerTest < ActionDispatch::IntegrationTest
  ...
  describe 'unauthenticated request' do
    ...
    it { response.must_redirect_to push_type_auth.new_user_session_path }
  end

  describe 'authenticated request' do
    before do
      post push_type_auth.user_session_path, params: { user: { email: current_user.email, password: 'password' } }
