module PushType
  module AdminHelper
    ...
    def asset_pack_path(name, **options)
      asset_path(PushType::Webpack.lookup(name), **options)
    end
    ...
    def javascript_pack_tag(name, **options)
      javascript_include_tag(PushType::Webpack.lookup("#{name}#{compute_asset_extname(name, type: :javascript)}"), **options)
    end
    ...
    def stylesheet_pack_tag(name, **options)
      stylesheet_link_tag(PushType::Webpack.lookup("#{name}#{compute_asset_extname(name, type: :stylesheet)}"), **options)
    end
  end
end
